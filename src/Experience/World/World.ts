...
export default class World extends kokomi.Component {
    ...
    this.base.am.on("ready", async () => {
      ...
      document.querySelector(".btn-click-me")?.addEventListener("click", () => {
        const clickSound = new Howl({
          src: "Genshin/Audio/Genshin Impact [Duang].mp3",
        });
        clickSound.play();
        this.createDoor();
      });
  ...
  createDoor() {
    ...
    this.ro?.on("door-comeout", () => {
      const doorComeoutSound = new Howl({
        src: "Genshin/Audio/Genshin Impact [DoorComeout].mp3",
      });
    ...
    this.ro?.on("door-created", () => {
      ...
      enterEl?.addEventListener("click", () => {
        const enterSound = new Howl({
          src: "Genshin/Audio/Genshin Impact [DoorThrough].mp3",
        });
        enterSound.play();
        this.enter();
      });
  ...
  async startGame() {
    ...
    const progressTween = gsap.to(rule, {
      width: "94.4%",
      duration: 5,
    });
